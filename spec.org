...
** Type system
   ...
   - error :: a special error type, to ease distinction of errors from other values. This
     type can only be instantianted by the built-in =error= function.
...
*** Recoverable errors
    ...
    mechanism should be used for reporting and handling errors:
    #+begin_src bash
    ...
    Command blocks and built-in functions will report errors by returning values of the
    =error= type, instead of the expected return value.
