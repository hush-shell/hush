...
**** Commands
     ...
     Variables can be accessed by prefixing their identifier with =$=, and are expanded with
     the following rules:
     ...
     2. =array=: each element will be converted to a single argument, using the first and
        third rules. This way, arrays can be used to programmatically build lists of
        command arguments.
...
***** Redirection
      ...
      operators:
      - ~command < filename~: opens /stdin/ as a reference to the given file name.
      ...
      - ~command fd> fd2~ or ~command fd> filename~: opens =fd= as a reference to the same file
        of =fd2=, or as a reference to the given file name. =fd= defaults to =1= (/stdout/) when
        omitted. The target file is created if it doesn't exists, or truncated otherwise.
      - ~command fd>> file~: opens =fd= as a reference to the given file name. =fd1= defaults to
      ...
      File names may be supplied through variables, but not file descriptors:
      #+begin_src bash
        let var = 2
        { command > $var } # Redirects to a file named "2"
      #+end_src

      In contrary to traditional shells, redirection operators must be placed after all of
