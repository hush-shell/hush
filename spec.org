...
***** Command lookup
      ...
      If there is no such command, /Hush/ returns a =127= status, and outputs an error
...
***** Arguments
      ...
      3. =dict=, =function=: won't be converted, causing a [[#Panics][panic]] instead.
...
***** Piping
      ...
      /Hush/ awaits all processes to finish, producing the status code of the last command
      in the pipeline.
...
***** Errors
      ...
      captured with ~${}~, /Hush/ will automatically [[#Panics][panic]], halting the script execution. This
